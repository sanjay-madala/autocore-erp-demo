  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-4 p-4 lg:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="flex items-center gap-2"><h1 className="text-[22px] font-[700] tracking-[-0.03em]">F1 — New Vehicle Sale</h1><Badge variant="success" className="gap-1"><span className="h-1.5 w-1.5 rounded-full bg-emerald-600 animate-pulse" /> LIVE OBJECT</Badge><span className="hidden md:inline rounded-full bg-zinc-900 px-2 py-0.5 font-mono text-[10px] font-semibold text-white">lead → desk → F&I → delivery → GL</span></div>
      ...
      <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
              ...
              {STEPS.map((s,i)=>{
                ...
                return (
                  <div key={s.id} className={cn("flex gap-3 px-3 py-2.5", cur && "bg-[var(--accent-muted)]")}>
                    <span className={cn("mt-0.5 grid h-6 w-6 place-items-center rounded-full text-[11px] font-bold", done? "bg-emerald-600 text-white": cur? "bg-[var(--accent)] text-white":"bg-zinc-200 text-zinc-600")}>{done? <CheckCircle size={14} weight="fill" />: i+1}</span>
        ...
        <div className="space-y-4">
          {/* live controls */}
          <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-white shadow-sm">
            <div className="flex items-center justify-between border-b bg-[var(--surface-muted)] px-4 py-3"><span className="text-[12px] font-semibold inline-flex items-center gap-2"><Lightning size={14} className="text-amber-500" /> Live pencil — &lt;500ms</span><span className="rounded-full bg-emerald-500 px-2 py-0.5 text-[11px] font-bold text-white">p95 42ms</span></div>
            ...
            <div className="px-4 pb-4 flex flex-wrap gap-2">
              ...
              <div className="flex gap-1">
                {["VSC","GAP","Tire","Dent"].map(p=>(
                  <button key={p} onClick={()=>toggleFi(deal.id,p)} className={cn("rounded-full border px-2.5 py-1 text-[11px] font-medium", deal.fiMenu[p]? "bg-emerald-600 text-white border-emerald-600":"bg-white")}>{p} {deal.fiMenu[p]? "✓":"+"}</button>
                ))}
              </div>
              ...
              <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 gap-1" onClick={()=>deliverDeal(deal.id)} disabled={deal.stage==="delivered"}>DELIVER → GL post <ShieldCheck size={12} weight="fill" /></Button>
            </div>
          </div>
